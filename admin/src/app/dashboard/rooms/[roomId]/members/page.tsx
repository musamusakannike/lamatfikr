...
import type { AdminRoomMember, AdminRoomMembersResponse } from "@/types/admin-community-room";

type DeletedFilter = "active" | "deleted" | "all";

type RoleFilter = "all" | "owner" | "admin" | "member";

type StatusFilter = "all" | "pending" | "approved" | "rejected" | "awaiting_payment";
...
export default function AdminRoomMembersPage() {
  ...
  const [data, setData] = useState<AdminRoomMembersResponse | null>(null);
  ...
  const queryString = useMemo(() => {
    ...
    return p.toString();
  }, [page, limit, deleted, role, status]);
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const res = await apiClient.get<AdminRoomMembersResponse>(`/admin/rooms/${roomId}/members?${queryString}`);
      setData(res);
    } catch (e) {
  ...
  const userName = (u: AdminRoomMember["userId"]) => {
  ...
  const renderStatus = (s: string) => {
    if (s === "pending") return t("adminCommunityRoom", "statusPending");
    if (s === "approved") return t("adminCommunityRoom", "statusApproved");
    if (s === "rejected") return t("adminCommunityRoom", "statusRejected");
    if (s === "awaiting_payment") return t("adminCommunityRoom", "statusAwaitingPayment");
    return s;
  };
  ...
  const setMemberRole = async (m: AdminRoomMember) => {
    const nextRole = window.prompt(t("adminCommunityRoom", "btnSetRole"), m.role);
    if (!nextRole) return;
    await mutate(m._id, async () => {
      await apiClient.patch(`/admin/rooms/${roomId}/members/${m._id}/role`, { role: nextRole });
    });
  };

  const setMemberStatus = async (m: AdminRoomMember) => {
    const nextStatus = window.prompt(t("adminCommunityRoom", "btnSetStatus"), m.status);
    if (!nextStatus) return;
    await mutate(m._id, async () => {
      await apiClient.patch(`/admin/rooms/${roomId}/members/${m._id}/status`, { status: nextStatus });
    });
  };

  const removeMember = async (m: AdminRoomMember) => {
    if (!window.confirm(t("adminCommunityRoom", "confirmRemove"))) return;
    await mutate(m._id, async () => {
      await apiClient.post(`/admin/rooms/${roomId}/members/${m._id}/remove`);
    });
  };
  ...
  return (
    <div className={cn("space-y-4", isRTL ? "text-right" : "text-left")}>
      <div>
        ...
        <div className="text-xs text-(--text-muted) font-mono">{roomId}</div>
      </div>
      ...
      <div className={cn("flex gap-3 flex-wrap", isRTL ? "flex-row-reverse" : "flex-row")}>
        ...
        >
          <option value="all">{t("adminCommunityRoom", "filterAll")}</option>
          <option value="pending">{t("adminCommunityRoom", "statusPending")}</option>
          <option value="approved">{t("adminCommunityRoom", "statusApproved")}</option>
          <option value="rejected">{t("adminCommunityRoom", "statusRejected")}</option>
          <option value="awaiting_payment">{t("adminCommunityRoom", "statusAwaitingPayment")}</option>
        </select>
                ...
                {(data.members ?? []).map((m) => {
                  ...
                  return (
                    <tr key={m._id} className="border-t border-(--border)">
                      ...
                      <td className="px-3 py-3 text-center">{renderStatus(m.status)}</td>
                      ...
                      <td className="px-3 py-3">
                        <div className={cn("flex gap-2 justify-center flex-wrap", isRTL ? "flex-row-reverse" : "flex-row")}>
                          ...
                          <button
                            disabled={isBusy}
                            onClick={() => setMemberStatus(m)}
                            className="px-2 py-1 rounded border border-(--border) bg-(--bg) disabled:opacity-50"
                          >
                            {t("adminCommunityRoom", "btnSetStatus")}
                          </button>
                          <button
