...
import type { AdminRoomMessage, AdminRoomMessagesResponse } from "@/types/admin-community-room";

type DeletedFilter = "active" | "deleted" | "all";

export default function AdminRoomMessagesPage() {
  ...
  const [data, setData] = useState<AdminRoomMessagesResponse | null>(null);
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const res = await apiClient.get<AdminRoomMessagesResponse>(`/admin/rooms/${roomId}/messages?${queryString}`);
      setData(res);
    } catch (e) {
  ...
  const senderName = (s: AdminRoomMessage["senderId"]) => {
  ...
  const onDelete = async (m: AdminRoomMessage) => {
    await mutate(m._id, async () => {
      await apiClient.post(`/admin/rooms/${roomId}/messages/${m._id}/delete`);
    });
  };

  const onRestore = async (m: AdminRoomMessage) => {
    await mutate(m._id, async () => {
      await apiClient.post(`/admin/rooms/${roomId}/messages/${m._id}/restore`);
    });
  };
