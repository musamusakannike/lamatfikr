...
import type { AdminPostsListResponse, AdminPostItem } from "@/types/admin-content";
...
export default function AdminPostsPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError(null);
      try {
        const res = await apiClient.get<AdminPostsListResponse>(`/admin/content/posts?${queryString}`);
        if (!mounted) return;
        setData(res);
      } catch (e) {
        if (!mounted) return;
  ...
  const onDelete = async (post: AdminPostItem) => {
    if (!window.confirm(t("adminContent", "confirmDelete"))) return;
    setBusyId(post._id);
    try {
      await apiClient.post(`/admin/content/posts/${post._id}/delete`);
      setPage(1);
    } catch (e) {
      setError(getErrorMessage(e));
    } finally {
  ...
  const onRestore = async (post: AdminPostItem) => {
    if (!window.confirm(t("adminContent", "confirmRestore"))) return;
    setBusyId(post._id);
    try {
      await apiClient.post(`/admin/content/posts/${post._id}/restore`);
      setPage(1);
    } catch (e) {
      setError(getErrorMessage(e));
    } finally {
  ...
  return (
    <div className={cn("space-y-4", isRTL ? "text-right" : "text-left")}>
      <div>
        <h1 className="text-2xl font-bold text-(--text)">{t("adminContent", "postsTitle")}</h1>
      </div>
      ...
      <div className={cn("flex gap-3 flex-wrap", isRTL ? "flex-row-reverse" : "flex-row")}>
        <input
          ...
          placeholder={t("adminContent", "searchPlaceholderPosts")}
                  ...
                  return (
                    <tr key={p._id} className="border-t border-(--border)">
                      ...
                      <td className={cn("px-3 py-3 max-w-[520px]", isRTL ? "text-right" : "text-left")}>
                        <div className="text-(--text) truncate">{p.contentText || "-"}</div>
                      </td>
                      <td className="px-3 py-3 text-center">{p.mediaCount ?? 0}</td>
