...
export default function FollowersPage() {
    ...
    const fetchFollowers = async (pageNum = 1) => {
        try {
            ...
            const response = await socialApi.getFollowers(undefined, pageNum, 20);
    ...
    const fetchFollowing = async () => {
        try {
            const response = await socialApi.getFollowing(undefined, 1, 1000);
            const ids = new Set(response.following.map((user) => user._id));
            setFollowingIds(ids);
        } catch (error) {
            ...
            <main className={cn("pt-16", isRTL ? "lg:pr-64" : "lg:pl-64")}>
                <div className="max-w-2xl mx-auto p-4 space-y-6">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-2xl font-bold">{t.followersTitle[language]}</h1>
                            <p className="text-sm text-(--text-muted)">
                                {followers.length} {followers.length === 1 ? t.followerCount[language] : t.followerCountPlural[language]}
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="relative">
                        ...
                        <input
                            type="text"
                            placeholder={t.searchFollowers[language]}
                            value={searchQuery}
                    ...
                    {isLoading ? (
                        <div className="flex flex-col items-center justify-center py-12">
                            ...
                            <p className="text-(--text-muted)">{t.loadingFollowers[language]}</p>
                        </div>
                        ...
                        <Card>
                            <CardContent className="flex flex-col items-center justify-center py-12">
                                ...
                                <h3 className="text-lg font-semibold mb-2">
                                    {searchQuery ? tCommon.search[language] : t.noFollowers[language]}
                                </h3>
