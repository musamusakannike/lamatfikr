...
export function EditProfileModal({ isOpen, onClose, onProfileUpdate }: EditProfileModalProps) {
  ...
  const [activeSection, setActiveSection] = useState<"basic" | "personal" | "professional" | "interests" | "photos">("basic");
  ...
  const addLanguage = () => {
    if (newLanguage.trim() && !formData.languagesSpoken.includes(newLanguage.trim()) && formData.languagesSpoken.length < 20) {
  ...
  const removeLanguage = (language: string) => {
    setFormData((prev) => ({ ...prev, languagesSpoken: prev.languagesSpoken.filter((l) => l !== language) }));
  };
